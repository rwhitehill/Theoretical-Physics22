...
The Laurent series of $G(x,z)$ (centered at the origin) is given as
\begin{eqnarray}
    \label{eq:Luarent-G}
    G(x,z) = \sum_{n=-\infty}^{\infty} \Big[ \frac{1}{2 \pi i} \oint_{C} \frac{G(x,z')}{z'^{n+1}} \dd{z'} \Big] z^{n}
.\end{eqnarray}
Thus, since we want the coefficient of this Laurent series to be the Bessel functions, we must have that
\begin{eqnarray}
    \label{eq:generating-func}
    \eqbox{
    G(x,z) = e^{(x/2)(z - 1/z)} 
}
.\end{eqnarray}
...
We can promote $x \mapsto z$ such that 
\begin{eqnarray}
